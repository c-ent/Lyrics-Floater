...
LYRICS = [
    (0.00, {"linger": [
        (0.00, "Kiss"), (0.38, " me"), (0.70, " like"), (0.98, " you"),
        (1.30, " know"), (1.88, " this"), (2.47, " is"), (3.05, " good"),
        (3.70, "bye"),
    ]}, None),

    (5.14, {"words": [
        (5.14, "We"), (5.71, " could"), (6.09, " try"), (6.66, " to"),
        (6.96, " make"), (7.32, " it"), (7.62, " all"), (7.93, " the"),
        (8.25, " way"), (8.80, " out"),
    ]}, None, 420),

    (9.68, {"words": [
        (9.68, "Grab"), (10.02, " me"), (10.32, " like"), (10.60, " my"),
        (10.92, " life"), (11.55, " is"), (12.10, " on"), (12.70, " the"),
        (13.32, " line"),
    ]}, None, 400),

    (14.82, {"words": [
        (14.82, "We"), (15.40, " decide"), (16.26, " to"), (16.54, " make"),
        (16.91, " it"), (17.19, " all"), (17.52, " the"), (17.83, " way"),
        (18.43, " out"),
    ]}, None, 420),

    (19.27, {"linger": [
        (19.27, "My"), (19.62, " heart"), (19.96, " in"), (20.23, " your"),
        (20.52, " hands"), (21.69, " despite"), (22.26, " all"),
        (22.56, " of"), (22.87, " my"), (23.22, " plans"),
    ]}, None),
    (20.70, {"tinker": ["heart", "hands", "plans", "make it count"]}, None),

    (24.39, {"words": [
        (24.39, "We"), (24.97, " could"), (25.34, " try"), (25.86, " to"),
        (26.17, " make"), (26.47, " it,"), (26.81, " make"), (27.10, " it"),
        (27.42, " all"), (27.96, " count"),
    ]}, None, 440),

    (28.87, {"slam": [
        (28.87, "Kiss"), (29.21, " me"), (29.53, " like"), (29.80, " you"),
        (30.08, " know"), (30.76, " this"), (31.32, " is"), (31.89, " good"),
        (32.47, "bye"),
    ]}, 4.0),
]
...
SHOUT_W           = 280
SHOUT_H           = 72
SHOUT_FONT_SIZE   = FONT_SIZE

# ── tinker swarm (many small windows bouncing around)
# They stay alive until the final slam ends.
TINKER_COUNT       = 10
TINKER_SPAWN_MS    = 40
...
TINKER_W           = 200
TINKER_H           = 64
TINKER_FONT_SIZE   = FONT_SIZE
TINKER_SPEED_MIN   = 0.25
TINKER_SPEED_MAX   = 0.55
# Keep tinkers out of the center band where main lyrics sit
TINKER_KEEPOUT_W   = 720        # clear zone width around screen center
TINKER_KEEPOUT_H   = 380        # clear zone height around screen center
...
def play_audio(path):
    try:
        ...
        print("No audio: pip install pygame")
...
def timed_full_text(words):
    """Join timed fragments into the full display string."""
    return "".join(frag for _, frag in words)


def _resize_typed_window(label, win):
    """Grow a typing window to fit current label text, keep it centered."""
...
def _type_chars(label, win, text, speed_ms, on_done=None):
    def step(i=0):
        try:
            ...
            _resize_typed_window(label, win)
        except tk.TclError:
            return
        if i < len(text):
...
def _type_timed(label, win, words, song_origin, on_done=None, resize=True):
    """Reveal text synced to absolute song times.

    words: [(abs_sec, fragment), ...] — fragments include leading spaces.
    Characters inside each fragment are paced evenly until the next word.
    """
    if not words:
        if on_done:
            on_done()
    ...
    # Build (abs_t, text_so_far) events — one per character
    events = []
    prefix = ""
    for i, (t0, frag) in enumerate(words):
        if not frag:
            continue
        if i + 1 < len(words):
            t1 = words[i + 1][0]
        else:
            t1 = t0 + max(0.25, len(frag) * 0.06)
        span = max(t1 - t0, 0.01)
        n = len(frag)
        for j, ch in enumerate(frag):
            tt = t0 + span * j / n
            prefix += ch
            events.append((tt, prefix))
    ...
    def show_at(idx, text, is_last):
        def show():
            try:
                ...
                return
            if is_last and on_done:
                on_done()
        delay_ms = max(0, int((song_origin + events[idx][0] - time.time()) * 1000))
        win.after(delay_ms, show)

    for idx, (_, text) in enumerate(events):
        show_at(idx, text, idx == len(events) - 1)
...
class SentenceWindow:
    ...
    def __init__(self, root, sentence, hold, w=LINE_W, type_speed=None,
        ...
        if words and song_origin is not None:
            _type_timed(self.label, self.win, words, song_origin,
                        on_done=self._start_hold)
        else:
            speed_ms = int((type_speed or TYPE_SPEED) * 1000)
            _type_chars(self.label, self.win, sentence, speed_ms,
                        on_done=self._start_hold)
...
class SlamWindow:
    ...
    def __init__(self, root, text, hold, type_speed=0.08,
        ...
        w  = self.WIDTH
        wrap = w - LINE_PAD * 2
        full = timed_full_text(words) if words else text
        ...
        if words and song_origin is not None:
            _type_timed(self.label, self.win, words, song_origin,
                        on_done=self._start_hold, resize=False)
        else:
            speed_ms = int(type_speed * 1000)
            self._type_fixed(full, speed_ms)
    ...
    def close(self):
        ...
        if self in _active_sentence_windows:
            _active_sentence_windows.remove(self)
        # End of song payoff — finally tear down the lingering tinker swarm
        ...
        for i, entry in enumerate(LYRICS):
            ...
            if isinstance(line, dict) and "shout" in line:
                ...
                if isinstance(payload, list):
                    def show(words=payload, h=hold, origin=start):
                        SentenceWindow(
                            root, timed_full_text(words), h, w=520,
                            centered=True, font_size=FONT_SIZE,
                            words=words, song_origin=origin,
                        )
                    root.after(0, show)
                else:
                    def show(s=payload, h=hold):
                ...
                if isinstance(payload, list):
                    def show(words=payload, h=hold, origin=start):
                        SlamWindow(root, timed_full_text(words), h,
                                   words=words, song_origin=origin)
                    root.after(0, show)
                else:
                    def show(s=payload, h=hold):
                ...
                words = line["words"]
                def show(words=words, h=hold, ww=w, origin=start):
                    SentenceWindow(
                        root, timed_full_text(words), h, w=ww,
                        words=words, song_origin=origin,
                    )
                root.after(0, show)
