...
LYRICS = [
    (0.00,  "Every night like the one before",                None, 380, 0.038),
    (1.81,  "Dream of you from like 1 to 4",                  None, 380, 0.050),
    (3.70,  "Positively and truly sure",                      None, 340, 0.034),
    (5.35,  {"shout": "Nobody's wanted somebody more"},       None),
    (7.41,  "It's a thing that i can't ignore",               None, 360, 0.038),
    (9.26,  "Tell your friends that you're mine, i'm yours",  None, 420, 0.022),
    (11.17, "With the hand on my heart, i swore",             None, 380, 0.038),
    (12.78, {"shout": "Nobody's wanted somebody more"},       None),
    (15.01, "It's a thing that i can't ignore",               None, 360, 0.038),
    (16.82, "Tell your friends that you're mine, i'm yours",  None, 420, 0.022),
    (18.70, {"linger": "With a hand on my heart, i swore"},   None),
    (21.20, {"tinker": ["i swore", "mine", "yours", "more", "heart"]}, None),

    (30.61, ["I'm going crazy,", "i'm going mad"],
            0.45, 2.0, (300, 160), 0, 48, False, 0.05),

    (34.43, {"slam": "I want you, baby, so bad"},             3.5),
]
...
TINKER_H           = 64
TINKER_FONT_SIZE   = 20
TINKER_SPEED_MIN   = 1.8
TINKER_SPEED_MAX   = 4.5
...
class SentenceWindow:
    ...
    def __init__(self, root, sentence, hold, w=LINE_W, type_speed=None,
        ...
        if centered:
            ...
            x, y = random_top_pos(root, w)
        self.win.geometry(f"{w}x{LINE_PAD * 2}+{x}+{y}")
        ...
        self.label = tk.Label(
            ...
            justify="left" if not centered else "center",
        )
...
class SlamWindow:
    ...
    def __init__(self, root, text, hold, type_speed=0.08):
        ...
        _type_chars(self.label, self.win, text, speed_ms, on_done=self._start_hold)
...
def show_shout_collage(root, text, hold=None):
    """
    Flood the screen with many small boxes of `text`, popping one after another
    ...
    def spawn(i=0):
        ...
        # slight size jitter so the collage feels less grid-like
        w = SHOUT_W + random.randint(-20, 40)
        h = SHOUT_H + random.randint(-8, 16)
        ShoutTile(root, text, x, y, w=w, h=h)

        root.after(SHOUT_INTERVAL_MS, lambda: spawn(i + 1))
...
def show_tinker_swarm(root, phrases, hold=None, count=None):
    ...
    through the provided list (or a single string). Swarm runs until the next
    lyric closes all windows.
    """
        ...
        for i, entry in enumerate(LYRICS):
            ...
            if isinstance(line, dict) and "shout" in line:
                ...
                def show(s=line["linger"], h=hold):
                    SentenceWindow(root, s, h, w=460, type_speed=0.07,
                                   centered=True, font_size=40)
                root.after(0, show)
